...
public class SearchingForAPositionUsingBS {
    ...
    public static boolean occurs(int[] arr, int x){
        for(int i = 0; i < arr.length; i++){
    ...
    public static int[] getElementAndClassifyIt(int[] arr){
        ...
        while(x >= 0){  
            ...
            x = sc.nextInt();
            if(x < 0) break;
            if(occurs(arr, x)) arr = newArray(arr, x);   //načtu prvek a zakládám mu nové pole
            else System.out.println("Zadaný prvek se v posloupnosti již vyskytuje"); 
        }
        return arr; 
    }
